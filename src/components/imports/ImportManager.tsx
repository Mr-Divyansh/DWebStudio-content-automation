...
export const ImportManager: React.FC<ImportManagerProps> = ({ onImportCompleted }) => {
  ...
  const handleInstagramHtmlImport = async () => {
    try {
      setIsProcessing(true);
      setImportNotice(null);
      const res = await api.importInstagramHtml();
      setImportNotice({
        type: 'success',
        message: 'Instagram HTML export imported and analyzed!',
        details: res.data,
      });
      await loadData();
      onImportCompleted();
      ...
      setImportNotice({
        type: 'error',
        message: err?.message || 'Instagram HTML import failed',
      });
    } finally {
      setIsProcessing(false);
    }
    ...
    <div className="space-y-6">
              ...
              <div className="font-mono text-[11px] text-[#A0AEC0]">
                {importNotice.details.conversationCount !== undefined && (
                  <span>
                    ...
                    {importNotice.details.messageCount !== undefined && ` | Messages: ${importNotice.details.messageCount}`}
                    {importNotice.details.filesParsed !== undefined && ` | Parsed: ${importNotice.details.filesParsed}`}
                    {importNotice.details.filesFailed !== undefined && importNotice.details.filesFailed > 0 && ` | Failures: ${importNotice.details.filesFailed}`}
                    {importNotice.details.duplicatesSkipped !== undefined && importNotice.details.duplicatesSkipped > 0 && ` | Duplicates Skipped: ${importNotice.details.duplicatesSkipped}`}
                  </span>
                )}
              </div>
            )}
      ...
      {activeTab === 'instagram' && (
        <div className="space-y-6">
          ...
          {/* Method 1b: Extracted Instagram HTML Export */}
          <div className="p-5 rounded-xl bg-[#0E131A] border border-[#1C232D] space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-[#F4F1EA] flex items-center gap-2">
                  <FileCode2 className="w-4 h-4 text-[#2F7EF2]" />
                  Instagram HTML Export
                  <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-[#12241A] border border-[#238636] text-[#7EE787]">
                    Supported
                  </span>
                </h3>
                <p className="text-xs text-[#8C98A9] mt-0.5">
                  Reads the already-extracted export at{' '}
                  <code className="bg-[#141A22] text-[#93C5FD] px-1.5 py-0.5 rounded font-mono">
                    data/instagram/extracted/Instagram-Leads/
                  </code>{' '}
            ...
            {folderStatus?.html?.extractedAvailable ? (
              <div className="p-3.5 rounded-lg bg-[#141F2D] border border-[#2F7EF2]/40 flex items-center justify-between">
                <div>
                  <div className="text-xs font-semibold text-[#6FB2FF] flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-[#7EE787]" />
                    {folderStatus.html.conversationFiles} conversation file(s) detected
                  </div>
                  <p className="text-[11px] text-[#8C98A9] mt-0.5">
                    {folderStatus.html.htmlFiles} HTML files found · Parses locally, then runs the existing lead analysis pipeline.
                  </p>
                </div>
                <button
                  onClick={handleInstagramHtmlImport}
                  disabled={isProcessing}
                  className="px-4 py-2 rounded-lg bg-[#2F7EF2] hover:bg-[#2568cc] text-white text-xs font-semibold shadow-md shadow-[#2F7EF2]/20 disabled:opacity-50 cursor-pointer"
                >
                  {isProcessing ? 'Processing...' : 'Run Pipeline on Extracted HTML'}
                </button>
              </div>
            ) : (
              <div className="p-3.5 rounded-lg bg-[#12171F] border border-[#1E2734] text-xs text-[#8C98A9]">
                No extracted Instagram HTML export found at{' '}
                <code className="text-[#C3CAD6]">data/instagram/extracted/Instagram-Leads/</code>. Extract your Instagram data export there first.
              </div>
            )}
          </div>
